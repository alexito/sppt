...
public class SolicitudBean {
    ...
    private String pickUsuarioExternoIds(List<Usuario> lista, boolean is_insert, Solicitud solic) throws SQLException {
        ...
        if (is_insert) {
            ...
            if(solic.getNuevoUsuarioExterno() != null)
    ...
    public List<Solicitud> saveSolicitud() throws IOException, SQLException, ParseException, AddressException, Exception {
        ...
        solicitudRetorno.getDistanciaById().getLocalidadByIdOrigen().setId(solicitudRetorno.getDistanciaById().getLocalidadByIdDestino().getId());
        solicitudRetorno.getDistanciaById().getLocalidadByIdDestino().setId(solicitudRetorno.getDistanciaById().getLocalidadByIdOrigen().getId());
...
//      Address[] responder_a = new Address[1];
//      responder_a[0]=new InternetAddress("fpr.reina@gmail.com");
        ...
        solicitud = new Solicitud();
        int aux = solicitud.getDistanciaById().getLocalidadByIdOrigen().getId();
//    solicitudBean.solicitud.distanciaById.localidadByIdDestino.id
        solicitud.getDistanciaById().getLocalidadByIdOrigen().setId(solicitud.getDistanciaById().getLocalidadByIdDestino().getId());
        solicitud.getDistanciaById().getLocalidadByIdDestino().setId(aux);
        updateInfoSolicitudes();

        return listaSolicitudesAprobadas;
    }

    public List<Solicitud> saveSolicitudEmergenciaRetorno() throws IOException, SQLException, ParseException, AddressException, Exception {
        Usuario logged_user = Select.LoggedUser();
        solicitudEmergenciaRetorno.setUsuarioByIdUsuarioSolicita(logged_user);
        int h = solicitudEmergenciaRetorno.getFSalida().getHours();
        int m = solicitudEmergenciaRetorno.getFSalida().getMinutes();
        if ((h == 15 && m > 30) || h >= 16) {
            solicitudEmergenciaRetorno.setEstado(false);
        } else {
            solicitudEmergenciaRetorno.setEstado(true);
        }
        
        solicitudEmergenciaRetorno.setFRetorno(solicitudEmergenciaRetorno.getFSalida());
        

        solicitudEmergenciaRetorno.setFCreacion(new Date());
        solicitudEmergenciaRetorno.setCancelado(false);

        solicitudEmergenciaRetorno.setRetorno(true);
        solicitudEmergenciaRetorno.setIds_interno_retorno(pickUsuarioInternoIds(solicitudEmergenciaRetorno.getListaInternosSeleccionados_retorno(), true));
        solicitudEmergenciaRetorno.setIds_externo_retorno(pickUsuarioExternoIds(solicitudEmergenciaRetorno.getListaExternosSeleccionados_retorno(), true, solicitudEmergenciaRetorno));
        
        solicitudEmergenciaRetorno.setEmergencia(true);
        String sol_id = Insert.InsertSolicitudEmergencia(solicitudEmergenciaRetorno);

        if (!solicitudEmergenciaRetorno.getEstado()) {
            emailSender es = new emailSender();
            listaEnfermeros = Select.selectUsuarios("enfermeros");
            ...
            int id_aprobador = Select.selectUsuarioIDByEMPLFAPR(solicitudEmergenciaRetorno.getUsuarioByIdUsuarioSolicita().getCodapr());
            String email = Select.selectUsuarioEmailById(id_aprobador);
            enviar_a[0] = new InternetAddress(email);

            String origen = Select.selectLocalidadNombreById(solicitudEmergenciaRetorno.getDistanciaById().getLocalidadByIdOrigen().getId());
            String destino = Select.selectLocalidadNombreById(solicitudEmergenciaRetorno.getDistanciaById().getLocalidadByIdDestino().getId());

            //Envia al aprobador
            es.send(enviar_a, responder_a, "Solicitud Especial", "La solicitud Numero: " + sol_id + " necesita su aprobacion. Los datos son los siguientes:\n"
                    + "\nNombre del solicitante: " + usuario.getNombrecompleto()
                    + "\nOrigen: " + origen + " \nDestino: " + destino + " \nDistancia: " + solicitud.getDistanciaById().getDistancia()
                    + "\nFecha Salida: " + (1900 + solicitudEmergenciaRetorno.getFSalida().getYear()) + "-" + (solicitudEmergenciaRetorno.getFSalida().getMonth() + 1) + "-" + solicitudEmergenciaRetorno.getFSalida().getDate() + " " + solicitudEmergenciaRetorno.getFSalida().getHours() + ":" + solicitudEmergenciaRetorno.getFSalida().getMinutes()
                    + "\nFecha Llegada " + (1900 + solicitudEmergenciaRetorno.getFLlegada().getYear()) + "-" + (solicitudEmergenciaRetorno.getFLlegada().getMonth() + 1) + "-" + solicitudEmergenciaRetorno.getFLlegada().getDate() + " " + solicitudEmergenciaRetorno.getFLlegada().getHours() + ":" + solicitudEmergenciaRetorno.getFLlegada().getMinutes()
                    + "");
            ...
            es.send(responder_a, responder_a_defecto, "Solicitud Especial", "Se ha generado una nueva Solicitud de Transporte con id Numero: " + sol_id + ". Los datos son los siguientes:\n"
                    + "\nNombre del solicitante: " + usuario.getNombrecompleto()
                    + "\nOrigen: " + origen + " \nDestino: " + destino + " \nDistancia: " + solicitudEmergenciaRetorno.getDistanciaById().getDistancia()
                    + "\nFecha Salida: " + (1900 + solicitudEmergenciaRetorno.getFSalida().getYear()) + "-" + (solicitud.getFSalida().getMonth() + 1) + "-" + solicitudEmergenciaRetorno.getFSalida().getDate() + " " + solicitudEmergenciaRetorno.getFSalida().getHours() + ":" + solicitudEmergenciaRetorno.getFSalida().getMinutes()
                    + "\nFecha Llegada " + (1900 + solicitudEmergenciaRetorno.getFLlegada().getYear()) + "-" + (solicitudEmergenciaRetorno.getFLlegada().getMonth() + 1) + "-" + solicitudEmergenciaRetorno.getFLlegada().getDate() + " " + solicitudEmergenciaRetorno.getFLlegada().getHours() + ":" + solicitudEmergenciaRetorno.getFLlegada().getMinutes()
                    + "");
        }

        solicitudEmergenciaRetorno = new Solicitud();

        return listaSolicitudesAprobadas;
    }
    
    public List<Solicitud> saveSolicitudRetorno() throws IOException, SQLException, ParseException, AddressException, Exception {
        Usuario logged_user = Select.LoggedUser();
        solicitudRetorno.setUsuarioByIdUsuarioSolicita(logged_user);
        int h = solicitudRetorno.getFSalida().getHours();
        int m = solicitudRetorno.getFSalida().getMinutes();
        if ((h == 15 && m > 30) || h >= 16) {
            solicitudRetorno.setEstado(false);
        } else {
            solicitudRetorno.setEstado(true);
        }

        if (!solicitudRetorno.getRetorno()) {
            solicitudRetorno.setFRetorno(new Date());
        }

        solicitudRetorno.setFCreacion(new Date());
        solicitudRetorno.setCancelado(false);

        solicitudRetorno.setRetorno(true);
        solicitudRetorno.setIds_interno_retorno(pickUsuarioInternoIds(solicitudRetorno.getListaInternosSeleccionados_retorno(), false));
        solicitudRetorno.setIds_externo_retorno(pickUsuarioExternoIds(solicitudRetorno.getListaExternosSeleccionados_retorno(), false, solicitudRetorno));
//    solicitudRetorno.setIds_interno(pickUsuarioInternoIds(solicitud.getListaInternosSeleccionados(), true));
//    solicitudRetorno.setIds_externo(pickUsuarioExternoIds(solicitud.getListaExternosSeleccionados(), true, solicitud));
//    
//        solicitudRetorno.setIds_interno_retorno(solicitud.getIds_interno());
//        solicitudRetorno.setIds_externo_retorno(solicitud.getIds_externo());

        String sol_id = Insert.InsertSolicitud(solicitudRetorno);
...
//      responder_a[0]=new InternetAddress("fpr.reina@gmail.com");
            ...
            int id_aprobador = Select.selectUsuarioIDByEMPLFAPR(solicitudRetorno.getUsuarioByIdUsuarioSolicita().getCodapr());
            String email = Select.selectUsuarioEmailById(id_aprobador);
            enviar_a[0] = new InternetAddress(email);

            String origen = Select.selectLocalidadNombreById(solicitudRetorno.getDistanciaById().getLocalidadByIdOrigen().getId());
            String destino = Select.selectLocalidadNombreById(solicitudRetorno.getDistanciaById().getLocalidadByIdDestino().getId());

            //Envia al aprobador
            es.send(enviar_a, responder_a, "Solicitud Especial", "La solicitud Numero: " + sol_id + " necesita su aprobacion. Los datos son los siguientes:\n"
                    + "\nNombre del solicitante: " + usuario.getNombrecompleto()
                    + "\nOrigen: " + origen + " \nDestino: " + destino + " \nDistancia: " + solicitud.getDistanciaById().getDistancia()
                    + "\nFecha Salida: " + (1900 + solicitudRetorno.getFSalida().getYear()) + "-" + (solicitudRetorno.getFSalida().getMonth() + 1) + "-" + solicitudRetorno.getFSalida().getDate() + " " + solicitudRetorno.getFSalida().getHours() + ":" + solicitudRetorno.getFSalida().getMinutes()
                    + "\nFecha Llegada " + (1900 + solicitudRetorno.getFLlegada().getYear()) + "-" + (solicitudRetorno.getFLlegada().getMonth() + 1) + "-" + solicitudRetorno.getFLlegada().getDate() + " " + solicitudRetorno.getFLlegada().getHours() + ":" + solicitudRetorno.getFLlegada().getMinutes()
                    + "");
            ...
            es.send(responder_a, responder_a_defecto, "Solicitud Especial", "Se ha generado una nueva Solicitud de Transporte con id Numero: " + sol_id + ". Los datos son los siguientes:\n"
                    + "\nNombre del solicitante: " + usuario.getNombrecompleto()
                    + "\nOrigen: " + origen + " \nDestino: " + destino + " \nDistancia: " + solicitudRetorno.getDistanciaById().getDistancia()
                    + "\nFecha Salida: " + (1900 + solicitudRetorno.getFSalida().getYear()) + "-" + (solicitud.getFSalida().getMonth() + 1) + "-" + solicitudRetorno.getFSalida().getDate() + " " + solicitudRetorno.getFSalida().getHours() + ":" + solicitudRetorno.getFSalida().getMinutes()
                    + "\nFecha Llegada " + (1900 + solicitudRetorno.getFLlegada().getYear()) + "-" + (solicitudRetorno.getFLlegada().getMonth() + 1) + "-" + solicitudRetorno.getFLlegada().getDate() + " " + solicitudRetorno.getFLlegada().getHours() + ":" + solicitudRetorno.getFLlegada().getMinutes()
                    + "");
        }
    ...
    public List<Solicitud> saveSolicitudEmergencia() throws IOException, SQLException, ParseException {
        ...
        solicitudEmergencia.setIds_interno_retorno(solicitudEmergencia.getIds_interno());
        solicitudEmergencia.setIds_externo_retorno(solicitudEmergencia.getIds_externo());
    ...
    public String getMensaje() {
        ...
        Date hora_permitida = new Date();
        hora_permitida.setHours(15);
        hora_permitida.setMinutes(30);
        if (hora_actual.after(hora_permitida)) {
            mensaje = "La solicitud será gestionada el día de mañana";
        }
        return mensaje;
    }
